...
def main():
    ...
    parser.add_argument('action', choices=['quote', 'kline', 'industry', 'concept', 'flow', 'search', 'etf'],
                        help='操作类型')
    ...
    try:
        if args.action == 'quote':
            ...
            if not args.symbol:
                print("错误: 需要 --symbol 参数 (如 513180)")
                sys.exit(1)
            code = args.symbol
            prefix = "sh" if code[:2] in ("51", "50", "58") else "sz"
            df = ak.fund_etf_hist_sina(symbol=f"{prefix}{code}")
            if df.empty:
                print(f"未找到 ETF {code}")
            else:
                print(json.dumps(df.tail(5).to_dict(orient='records'), ensure_ascii=False, indent=2))
