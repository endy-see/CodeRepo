...
# OUTPUT_DIR="$SCRIPT_DIR/checkpoints/models_modernBERT_optimize1"
# OUTPUT_DIR="$SCRIPT_DIR/checkpoints/models_modernBERT_optimize2"  # Query, collapsed_label
# OUTPUT_DIR="$SCRIPT_DIR/checkpoints/models_modernBERT_optimize3"
OUTPUT_DIR="$SCRIPT_DIR/checkpoints/models_modernBERT_optimize4"    # Prompt, risk_type_label

mkdir -p $OUTPUT_DIR

# Create cache directory
CACHE_DIR="$SCRIPT_DIR/cache_modernBERT/query_sensitivity_for_risk_level"
mkdir -p $CACHE_DIR

# Set training parameters with optimized values
TRAIN_FILE="/cosmos/local/IndexQuality/ContentModels/DataAugmentation/data/CBSpam_v3/QuerySensitivity/data_all5.tsv"    # train_columns: "prompt,risk_type_label"
# TRAIN_FILE="/cosmos/local/IndexQuality/ContentModels/DataAugmentation/data/CBSpam_v3/QuerySensitivity/all_set_query_collapsed_label1.tsv"  # train_columns: "Query,risk_type_label"
...
ACCELERATE_CONFIG_FILE=accelerate_config.yaml accelerate launch \
    ...
    --gradient_accumulation_steps ${GRAD_ACCUM_STEPS} \
    modernBERT_04_07_optimize1.py \
    --train_file $TRAIN_FILE \
    --train_columns "prompt,risk_type_label" \
  ...
  # train_columns "prompt,risk_type_label" \
  # train_columns "Query,risk_type_label" \

        
# --mixed_precision bf16 \
# ./modernBERT_04_07_optimize1.sh
# pip install --upgrade torch torchvision transformers accelerator tensorboard
