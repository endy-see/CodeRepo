...
OUTPUT_DIR="$SCRIPT_DIR/checkpoints/models_modernBERT_hierarchical_classifier"
mkdir -p $OUTPUT_DIR

# Create cache directory
CACHE_DIR="$SCRIPT_DIR/cache_modernBERT/query_sensitivity_for_hierarchical_classifier"
mkdir -p $CACHE_DIR

# Set training parameters with optimized values
TRAIN_FILE='/cosmos/local/IndexQuality/ContentModels/DataAugmentation/data/CBSpam_v3/QuerySensitivity/data_all6_for_hierarchical1.tsv'
BATCH_SIZE=80                         # Reduced batch size for stability, default 64
NUM_EPOCHS=10
...
ACCELERATE_CONFIG_FILE=accelerate_config.yaml accelerate launch \
    ...
    --gradient_accumulation_steps ${GRAD_ACCUM_STEPS} \
    modernBERT_hierarchical_classifier.py \
    --train_file $TRAIN_FILE \
    --train_columns "prompt,collapsed_label,level2_label" \
