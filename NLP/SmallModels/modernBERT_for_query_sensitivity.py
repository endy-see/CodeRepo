...
os.environ[
    "NCCL_IB_TIMEOUT"] = "3600"  # only increased timeout is meaningless, because if the root cause is not resolved, the program will eventually crash
...
# Define class names
id2class = {0: 'Sensitive', 1: 'Informational', 2: 'Tolerant'}
        ...
        if accelerator.is_main_process:
            try:
                ...
                metadata = {
                    ...
                    "id2label": {0: "Sensitive", 1: "Informational", 2: "Tolerant"},
                    "label2id": {"Sensitive": 0, "Informational": 1, "Tolerant": 2}
                }
...
def training_function(args):
    ...
    # Create datasets
    train_dataset = QueryDataset(train_df, tokenizer, required_columns, max_length=args.max_length,
                                 cache_dir=args.cache_dir, name='train')
    val_dataset = QueryDataset(val_df, tokenizer, required_columns, max_length=args.max_length,
                               cache_dir=args.cache_dir, name='val')
    test_dataset = QueryDataset(test_df, tokenizer, required_columns, max_length=args.max_length,
                                cache_dir=args.cache_dir, name='test')
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                progress_bar.set_description(
                    f"Epoch {epoch + 1}/{args.num_epochs} - Loss: {avg_loss:.4f}")  # Step {step+1}/{(len(train_dataloader)//4)}  # 4 is GPU count
