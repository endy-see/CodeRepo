...
class PreprocessingDatasetTemplate(Dataset):
    ...
    def _load_memmap(self, data_path, memmap_path):
        ...
        df = pd.read_csv(data_path, sep='\t', header=None, names=['url', 'html_text', 'label', 'index'])
        for row in df.itertuples():
            url, html_text, label, idx = row.url, row.html_text, row.label, row.index
            index = str(idx)
            if type(html_text) != str or len(html_text.strip()) < 1:
                html_text = ''
            ...
            full_text_token = torch.tensor(self.tokenizer.encode(html_text, add_special_tokens=True,
                                                                 ...
                                                                 truncation=True))
            label = self._generate_label(raw_label, categories=1)
