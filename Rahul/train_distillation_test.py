...
data_dir = '/data/local/users/yanmeizhao/spam/'
...
class SpamMonoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # length, percentiles = self.calculate_lengths()
...
def eval_binary_classifier(model, dataloader, iter_num, is_mikhail=False, eval_april=False,
    ...
    for idx, (url_tokens, inputs, labels, pipelines) in enumerate(dataloader):
        ...
        if eval_only_llm and len(label_list) >= 400000 * 5:
            break
    ...
    for idx in range(5):
        ...
        thresholds_to_consider = [0.9762, 0.2381, 0.8175]
...
def infer(model, batch_size, infer_file):
    ...
    with open(infer_file, "r") as f:
        ...
        for idx, line in enumerate(f):
            url, text_tokens = line.strip().split("\t")

            url_tokens = torch.tensor(
                [tokenizer.cls_token_id] + tokenizer.encode(url, add_special_tokens=False, max_length=511,
                                                            padding="max_length", truncation=True))
    ...
    with open("/blob/rmayuranath/fy23_train_infer_results.tsv", "w+") as f:
        for url, scores, embedding in results:
            f.write(
                "{}\t{}\t{}\n".format(url, "!".join([str(x) for x in scores]), "!".join([str(x) for x in embedding])))
...
def train(model, batch_size, data_dir):
    ...
    if eval_oct_leakage or eval_mikhail_set or eval_april_leakage or eval_only:
        if gpu_id == 0:
            if eval_only:
                ...
                if eval_april_leakage:
                    ...
                    validation_file = "/scratch/singularity_webdata_ws01_eastus2_nfs/rmayuranath/data/v2/finetune/april_leakage_dedup_brand.tsv"
                else:
    ...
    if is_finetune:
        ...
        if is_bs_finetune:
            ...
            # valid_neg_file = os.path.join(data_dir, "negatives_27M_valid.tsv")
            train_pos_file = os.path.join(data_dir, "latest_training_data_positive_train_o1.csv")
            valid_pos_file = os.path.join(data_dir, "latest_training_data_positive_valid_o1.csv")
            train_neg_file = os.path.join(data_dir, "latest_training_data_negative_train_o1.csv")
            valid_neg_file = os.path.join(data_dir, "latest_training_data_negative_valid_o1.csv")
            # load the data
            ...
            # validation_file = "/scratch/singularity_webdata_ws01_eastus2_nfs/rmayuranath/data/v2/finetune/mikhail_bs/mikhail_set_evaluation_dedup.tsv"
            # validation_set = SpamMonoDataset(positive_file=validation_file, shuffle=False)
            # validation_loader = DataLoader(validation_set, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=4)
            ...
            # if train_pos_file.endswith("positives_finetune_train.tsv") and train_neg_file.endswith("negatives_27M_train.tsv"):
            #     train_dataset.pipeline_counts = {'Negative': 22214564, 'Other': 1142656, 'MGC': 480006, 'Media': 88758, 'KeywordStuffing': 284661, 'Empty': 6803, 'BrandStuffing': 1003128}
            # elif train_pos_file.endswith("positives_finetune_train_w_bs.tsv") and train_neg_file.endswith("negatives_27M_train.tsv"):
            #     train_dataset.pipeline_counts = {'Negative': 22214564, 'Other': 1142656, 'MGC': 480006, 'Media': 88758, 'KeywordStuffing': 284661, 'Empty': 6803, 'BrandStuffing': 1003651}
            # elif train_pos_file.endswith("positives_finetune_train_w_bs.tsv") and train_neg_file.endswith("negatives_27M_train_w_bs.tsv"):
            #     train_dataset.pipeline_counts = {'Negative': 23643991, 'Other': 1142656, 'MGC': 480006, 'Media': 88758, 'KeywordStuffing': 284661, 'Empty': 6803, 'BrandStuffing': 1003651}
    ...
    if not is_bs_finetune:
        # weights_path = os.path.join(data_dir, f"weights_all_{train_pos_file.split('/')[-1].split('.')[0]}_{train_neg_file.split('/')[-1].split('.')[0]}_{sum(desired_samples.values())}.pkl")
    ...
    while True:
        ...
        while True:
            ...
            for batch in train_loader:
                ...
                loss = loss_function(student_scores, labels.float(), pipelines)

                # backward pass, with gradient scaling if training in fp16
                # scaler.scale(loss).backward()
                loss.backward()
                # print("---->>> ", student_scores.grad, student_scores.shape, labels.shape)
...
if __name__ == "__main__":
    ...
    # ./train_distillation_test.sh --resume_from_checkpoint /scratch/workspaceblobstore/spam/logs/logs_distill_3e-4_model_checkpoint_72k_temperature_4.0/checkpoint_iter_135000 --tensorboard_dir v3_distill_contains_subtypes_mask_10_31_4_loss --finetune --finetune_dropout_replace_experiment --eval_april_leakage
