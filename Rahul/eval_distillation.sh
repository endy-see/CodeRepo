...
case $key in
    ...
    finetune_dropout_replace_experiment="$2"
    shift # past argument
    shift # past value
    ;;
    --batch_size)
    ...
    shift # past value
    ;;
    --zero_last_layer_grads)
    zero_last_layer_grads="$2"
    shift # past argument
    shift # past value
    ;;
    --temperature)
...
# build the command
command=("torchrun" "--standalone" "--nproc_per_node=4" "eval_distillation.py" "--tensorboard_dir" "$tensorboard_dir" "--freeze_encoder" "$freeze_encoder" "--enc_num_layers" "$enc_num_layers" "--average_embeddings" "$average_embeddings" "--temperature" "$temperature" "--alpha" "$alpha")

if [ "$finetune" == "True" ]; then
    command+=("--finetune")
fi

if [ "$bs_finetune" == "True" ]; then
    command+=("--bs_finetune")
fi
...
if [ "$eval_may_leakage" == "True" ]; then
    command+=("--eval_may_leakage")
fi
...
if [ "$zero_last_layer_grads" == "True" ]; then
