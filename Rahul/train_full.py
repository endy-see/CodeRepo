...
class SpamDataset(Dataset):
    ...
    def calculate_lengths(self):
        ...
        for index in range(len(self.memmap_positive) - 1):
        ...
        for index in range(len(self.memmap_negative) - 1):
            url_tokens, _, _, _ = self.memmap_negative[index]
            url_length = np.count_nonzero(url_tokens)
            # url_length = len(url_tokens) - np.argmax(url_tokens[::-1] != 0) + 1
            lengths.append(url_length)
...
eval_interval = 8000 if not is_bs_finetune else 12000  # how often to evaluate the model on the validation set
...
def train(model, batch_size, data_dir):
    ...
    if is_finetune:
        ...
        validation_loader_april = DataLoader(validation_set_april, batch_size=batch_size, shuffle=True, pin_memory=True,
                                             num_workers=4)
        ...
        while True:
            ...
            for batch in train_loader:
                ...
                if gpu_id == 0 and iter_num % eval_interval == 0 and iter_num != 0:
                    eval_only_llm = True if not eval_only else False
                    # eval_binary_classifier(model, valid_loader, iter_num, eval_only_llm=eval_only_llm)
                    if eval_only:
                        return
