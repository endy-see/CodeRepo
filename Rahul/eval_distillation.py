...
# average_embeddings = sys.argv[sys.argv.index('--average_embeddings') + 1] == 'True' if '--average_embeddings' in sys.argv else True
average_embeddings = True
eval_only = '--eval_only' in sys.argv
eval_oct_leakage = '--eval_oct_leakage' in sys.argv
eval_may_leakage = '--eval_may_leakage' in sys.argv
eval_april_leakage = '--eval_april_leakage' in sys.argv
eval_mikhail_set = '--eval_mikhail_set' in sys.argv
zero_last_layer_grads = '--zero_last_layer_grads' in sys.argv
        ...
        for idx, sample in enumerate(open(file_path, 'rb')):
            ...
            try:
                ...
                url_tokens = torch.tensor(
                    [self.tokenizer.cls_token_id] + self.tokenizer.encode(url, add_special_tokens=False, max_length=127,
...
class SpamMonoDataset(Dataset):
    ...
    def _load_memmap(self, file_path, memmap_path, count):
        ...
        for idx, sample in enumerate(open(file_path, 'rb')):
            ...
            sample = sample.decode('utf-8').strip().split('\t')
            url, pipelines, tokens, label, html, url_tokens = None, None, None, None, None, None
            ...
            if eval_oct_leakage:
                ...
                pipelines = "Negative" if label == 0 else "BrandStuffing,Empty"

            elif eval_may_leakage:
                url, url_tokens, tokens, label = sample
                label = int(label)
            ...
            assert url is not None and tokens is not None and pipelines is not None and label is not None, f"One of url, tokens, pipelines, or label is None. url: {url}, tokens: {tokens}, pipelines: {pipelines}, label: {label}"

            if not eval_may_leakage:
                url_tokens = torch.tensor(
                    [self.tokenizer.cls_token_id] + self.tokenizer.encode(url, add_special_tokens=False, max_length=511,
                                                                          padding="max_length", truncation=True))
            else:
                url_tokens = [int(t) for t in url_tokens.split('!')]
                url_tokens = torch.tensor(url_tokens, dtype=torch.int32)
    ...
    def calculate_lengths(self):
        ...
        for index in range(len(self.memmap_positive)):
            url_tokens, _, _, _ = self.memmap_positive[index]
            url_length = np.count_nonzero(url_tokens)
            # url_length = len(url_tokens) - np.argmax(url_tokens[::-1] != 0) + 1
            lengths.append(url_length)

        print("computed total number of urls: ", len(lengths))
        # Compute the average length and percentiles
        avg_length = np.mean(lengths)
        percentiles = [np.percentile(lengths, percentile) for percentile in
                       [50, 60, 70, 80, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100]]

        return avg_length, percentiles
...
class SpamModelV2(nn.Module):
    def __init__(self, enc_num_layers=1, combine_num_layers=1):
        ...
        if is_bs_finetune:
            # freeze all layers except last layer of fc
...
class SpamStudentModelV2(nn.Module):
    def __init__(self, enc_num_layers=1, combine_num_layers=1):
        super(SpamStudentModelV2, self).__init__()
        self.enc = AutoModel.from_pretrained("prajjwal1/bert-tiny")
        # keep only first n encoder layer
        self.enc.encoder.layer = self.enc.encoder.layer[:enc_num_layers]
        self.enc.embeddings.word_embeddings = nn.Embedding(119547, 128)
        self.enc.embeddings.apply(self._init_weights)

        self.url_enc = AutoModel.from_pretrained("prajjwal1/bert-tiny")
        self.url_enc.encoder.layer = self.url_enc.encoder.layer[:1]
        self.url_enc.embeddings.word_embeddings = nn.Embedding(119547, 128)
        self.url_enc.embeddings.apply(self._init_weights)

        self.combine = BertModel.from_pretrained("prajjwal1/bert-tiny")
        self.combine.encoder.layer = self.combine.encoder.layer[:combine_num_layers]
        # set to empty so dont waste space.
        self.combine.embeddings.word_embeddings = None
        self.combine.embeddings.position_embeddings = PositionalEncoding(NUM_SEQ + 1, 128)
...
teacher_model = SpamModelV2(enc_num_layers=int(enc_num_layers), combine_num_layers=1)
...
weight_decay = 1e-1
betas = (0.9, 0.95)
# learning_rate = 1e-5 if is_bs_finetune else 1e-4 # max learning rate
learning_rate = 3e-4
...
class DistillationLoss(nn.Module):
    ...
    def forward(self, student_scores, student_logits, teacher_logits, labels, pipelines):
        ...
        distillation_loss = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(student_logits / self.temperature, dim=1),
...
# data related
dtype = 'float16'
gradient_accumulation_steps = 4 if not is_bs_finetune else 1  # how many batches to accumulate gradients for
eval_interval = 2500 if not is_bs_finetune else 3000  # how often to evaluate the model on the validation set
log_interval = 500  # how often to log training information
...
if "--resume_from_checkpoint" in sys.argv:
    ...
    # note, we want to load the teacher model, not the student model.
    infer = "--infer" in sys.argv
    # if not (eval_april_leakage or eval_only or eval_mikhail_set or infer):
    if False:
        teacher_model.load_state_dict(new_state_dict, strict=True)
        print("teacher_model loaded from checkpoint path: ", checkpoint_path)
    else:
...
if ddp:
    ...
    teacher_model = DDP(teacher_model, device_ids=[gpu_id], find_unused_parameters=False if is_bs_finetune else True)
    print("teacher_model wrapped in DDP...")
...
def eval_binary_classifier_modified(model, dataloader, iter_num, is_mikhail=False, eval_april=False,
                                    eval_only_llm=False) -> None:
    # 1. Preparation
    tokenizer = BertTokenizer.from_pretrained('bert-base-multilingual-cased')
    rows_for_tsv = []
    fp_rows = []
    ...
    for idx, (url_tokens, inputs, labels, pipelines) in enumerate(dataloader):
        ...
        # Decode the url_tokens and inputs
        # Decode the url_tokens and inputs
        decoded_urls = tokenizer.batch_decode(url_tokens, skip_special_tokens=True)
        decoded_urls = [url.replace(' ', '') for url in decoded_urls]  # Remove spaces from the decoded URLs

        # Reshape the tensor to [batch_size, num_seq * 512]
        flattened_inputs = inputs.view(inputs.shape[0], -1)

        # Convert tensor to CPU and to list
        if isinstance(flattened_inputs, torch.Tensor):
            flattened_inputs = flattened_inputs.cpu().tolist()

        # Decode
        decoded_inputs = tokenizer.batch_decode(flattened_inputs, skip_special_tokens=True)
        decoded_inputs = [' '.join(text.split()) for text in
                          decoded_inputs]  # Remove redundant spaces from the decoded inputs

        # Get the "IsSpam" label and its corresponding model output score
        is_spam_labels = labels[:, -1].tolist()  # assuming the last label is "IsSpam"
        is_spam_scores = outputs[:, -1].tolist()

        for url, input_text, label, score in zip(decoded_urls, decoded_inputs, is_spam_labels, is_spam_scores):
            # Check if the "IsSpam" label is 1 and the model's prediction score is below the threshold
            if label == 1 and score < threshold:
                rows_for_tsv.append((url, input_text, score, label))
            if label == 0 and score > threshold:
                fp_rows.append((url, input_text, score, label))
    ...
    # sampled_rows = random.sample(rows_for_tsv, min(500, len(rows_for_tsv)))
    sampled_rows = rows_for_tsv

    # Write to a TSV file
    with open('spam_predictions_false_negatives_all.tsv', 'w', newline='') as tsvfile:
        writer = csv.writer(tsvfile, delimiter='\t')
        writer.writerow(["URL", "Input", "Output_Score", "Label"])
        writer.writerows(sampled_rows)

    with open('spam_predictions_false_positives_all.tsv', 'w', newline='') as tsvfile:
        writer = csv.writer(tsvfile, delimiter='\t')
        writer.writerow(["URL", "Input", "Output_Score", "Label"])
    ...
    print("TSV file with predictions saved to spam_predictions.tsv")


def eval_binary_classifier(model, dataloader, iter_num, is_mikhail=False, eval_april=False,
                           eval_only_llm=False) -> None:
    print("evaluating...")
    model.eval()  # set model to evaluation mode
    ...
    global eval_april_leakage
    eval_april_leakage = eval_april_leakage or eval_april
    ...
    for idx in range(5):
        ...
        if neg_indices:
            ...
            if eval_may_leakage:
                # randomly sample 11x as many negatives as positives
                neg_indices_sub = random.sample(neg_indices, min(30 * len(output_list_sub), len(neg_indices)))
        ...
        thresholds_to_consider = [0.9286]

        from sklearn.metrics import confusion_matrix

        for threshold in thresholds_to_consider:
            # Apply threshold
            predicted_labels = [1 if output > threshold else 0 for output in output_list_sub]

            # Calculate confusion matrix
            tn, fp, fn, tp = confusion_matrix(label_list_sub, predicted_labels).ravel()

            # Calculate precision (tp / (tp + fp))
            if tp + fp != 0:  # Avoid dividing by zero
                precision = tp / (tp + fp)
            else:
                precision = 0
            print(f"Precision at threshold {threshold} for label {spam_subtype_mapping[idx]}: {precision}")

            # Calculate false positive rate (fp / (fp + tn))
            if fp + tn != 0:  # Avoid dividing by zero
                fpr = fp / (fp + tn)
            else:
                fpr = 0
            print(f"FPR at threshold {threshold} for label {spam_subtype_mapping[idx]}: {fpr}")
...
def infer(model, batch_size, infer_file):
    results = []
    model = model.to(device)

    with open(infer_file, "r") as f:
        # read url, tokens, data line by line
        for idx, line in enumerate(f):
            url, url_tokens, text_tokens = line.strip().split("\t")

            # url_tokens = torch.tensor([tokenizer.cls_token_id] + tokenizer.encode(url, add_special_tokens=False, max_length=511, padding="max_length", truncation=True))
            # text tokens are already tokenized. read them to torch tensor.
            url_tokens = torch.tensor([int(x) for x in url_tokens.split("!")])
            text_tensor = torch.tensor([int(x) for x in text_tokens.split("!")])
            text_tensor = text_tensor.reshape(NUM_SEQ, MIN_SEQ_LENGTH)
            ...
            # get model outputs
            with torch.no_grad():
                scores, embedding = model(url_tokens.unsqueeze(0), text_tensor.unsqueeze(0))
                scores = F.sigmoid(scores)

            # write url, scores, embedding to results
            results.append((url, scores[0].tolist(), embedding[0].tolist()))

            if idx % 1000 == 0:
                print("processed {} lines".format(idx))

    # write results to file
    with open("/blob/rmayuranath/analysis/KRSet_SD_infer_results.tsv", "w+") as f:
        for url, scores, embedding in results:
            formatted_scores = "|".join(["{:.5f}".format(x) for x in scores])
            formatted_embedding = "|".join(["{:.5f}".format(x) for x in embedding])
            f.write("{}\t{}\t{}\n".format(url, formatted_scores, formatted_embedding))

    print("done writing results to file.")


def train(model, batch_size, data_dir):
    temperature = float(sys.argv[sys.argv.index("--temperature") + 1]) if "--temperature" in sys.argv else 5.0
    alpha = float(sys.argv[sys.argv.index("--alpha") + 1]) if "--alpha" in sys.argv else 0.95

    loss_function = DistillationLoss(temperature=temperature, alpha=alpha)
    ...
    if eval_oct_leakage or eval_mikhail_set or eval_april_leakage or eval_only or eval_may_leakage:
        if gpu_id == 0:
            if eval_only:
                print("evaluating on finetuning validation set...")
                ...
                validation_set = SpamDataset(valid_pos_file, valid_neg_file, shuffle=False)
            else:
                if eval_april_leakage:
                    print("evaluating on april leakage set...")
                    validation_file = "/scratch/singularity_webdata_ws01_eastus2_nfs/rmayuranath/data/v2/finetune/april_leakage_dedup_brand_.tsv"
                elif eval_may_leakage:
                    print("evaluating on may leakage set...")
                    validation_file = "/blob/rmayuranath/may_june_judgements.tsv"
                else:
                    validation_file = "/scratch/workspaceblobstore/spam/data/text/test_leakage_data_512x4.tsv" if eval_oct_leakage else "/scratch/singularity_webdata_ws01_eastus2_nfs/rmayuranath/data/v2/finetune/mikhail_bs/mikhail_set_evaluation_dedup.tsv"
            ...
            validation_loader = DataLoader(validation_set, batch_size=batch_size, shuffle=True, pin_memory=True,
                                           num_workers=4)
            eval_binary_classifier_modified(model, validation_loader, 0)
        else:
    ...
    if is_finetune:
        ...
        if is_bs_finetune:
            ...
            validation_file = "/scratch/singularity_webdata_ws01_eastus2_nfs/rmayuranath/data/v2/finetune/mikhail_bs/mikhail_set_evaluation_dedup.tsv"
            valid_dataset = validation_set = SpamMonoDataset(positive_file=validation_file, shuffle=False)
            validation_loader = DataLoader(validation_set, batch_size=batch_size, shuffle=True, pin_memory=True,
                                           num_workers=4)

        else:
            train_pos_file = os.path.join(data_dir, "positives_finetune_train_w_bs.tsv")
            valid_pos_file = os.path.join(data_dir, "positives_finetune_validation.tsv")
            train_neg_file = os.path.join(data_dir, "negatives_27M_train_w_bs.tsv")
            valid_neg_file = os.path.join(data_dir, "negatives_27M_valid.tsv")
            ...
            # load mikahil evaluation set
            validation_file = "/scratch/singularity_webdata_ws01_eastus2_nfs/rmayuranath/data/v2/finetune/mikhail_bs/mikhail_set_evaluation_dedup.tsv"
            validation_set = SpamMonoDataset(positive_file=validation_file, shuffle=False)
            validation_loader = DataLoader(validation_set, batch_size=batch_size, shuffle=True, pin_memory=True,
                                           num_workers=4)

            validation_file_april = "/scratch/singularity_webdata_ws01_eastus2_nfs/rmayuranath/data/v2/finetune/april_leakage_dedup_brand.tsv"
            validation_set_april = SpamMonoDataset(positive_file=validation_file_april, shuffle=False)
            validation_loader_april = DataLoader(validation_set_april, batch_size=batch_size, shuffle=True,
                                                 pin_memory=True, num_workers=4)

            # if pos file ends with positives_finetune_train.tsv and neg file ends with negatives_27M_train.tsv, then update pipeline counts.
            if train_pos_file.endswith("positives_finetune_train.tsv") and train_neg_file.endswith(
                    "negatives_27M_train.tsv"):
                train_dataset.pipeline_counts = {'Negative': 22214564, 'Other': 1142656, 'MGC': 480006, 'Media': 88758,
                                                 'KeywordStuffing': 284661, 'Empty': 6803, 'BrandStuffing': 1003128}
            elif train_pos_file.endswith("positives_finetune_train_w_bs.tsv") and train_neg_file.endswith(
                    "negatives_27M_train.tsv"):
                train_dataset.pipeline_counts = {'Negative': 22214564, 'Other': 1142656, 'MGC': 480006, 'Media': 88758,
                                                 'KeywordStuffing': 284661, 'Empty': 6803, 'BrandStuffing': 1003651}
            elif train_pos_file.endswith("positives_finetune_train_w_bs.tsv") and train_neg_file.endswith(
                    "negatives_27M_train_w_bs.tsv"):
                train_dataset.pipeline_counts = {'Negative': 23643991, 'Other': 1142656, 'MGC': 480006, 'Media': 88758,
                                                 'KeywordStuffing': 284661, 'Empty': 6803, 'BrandStuffing': 1003651}
    ...
    if not is_bs_finetune:
        ...
        if os.path.exists(weights_path):
            ...
            # delete keys of train_dataset.pipeline_counts if they are 0.
            for key in list(train_dataset.pipeline_counts.keys()):
                if train_dataset.pipeline_counts[key] == 0:
                    del train_dataset.pipeline_counts[key]

            print("train_dataset.pipeline_counts: ", train_dataset.pipeline_counts)
        ...
        while True:
            ...
            for batch in train_loader:
                ...
                if gpu_id == 0 and iter_num % eval_interval == 0 and iter_num != 0:
                    eval_binary_classifier(model, valid_loader, iter_num, eval_only_llm=False)
                    eval_binary_classifier(model, validation_loader, iter_num, is_mikhail=True)
                    # eval on april leakage
                    if not is_bs_finetune:
                        eval_binary_classifier(model, validation_loader_april, iter_num, eval_april=True)
                ...
                with ctx:
                    # student
                    student_scores, student_logits = model(url_tokens.to(device), data.to(device))

                with torch.no_grad():
                    # teacher
                    teacher_scores, teacher_logits = teacher_model(url_tokens.to(device), data.to(device))

                labels = labels.to(device)

                loss = loss_function(student_scores, student_logits, teacher_logits, labels.float(), pipelines)

                # backward pass, with gradient scaling if training in fp16
                scaler.scale(loss).backward()

                # if bs_finetune, set gradients of non first node of last layer to zero.
                # if is_bs_finetune and zero_last_layer_grads:
                #     if ddp:
                #         model.module.fc[-1].weight.grad[:, 1:] = 0
                #         model.module.fc[-1].bias.grad[1:] = 0
                #     else:
                #         model.fc[-1].weight.grad[:, 1:] = 0
                #         model.fc[-1].bias.grad[1:] = 0
                ...
                if gpu_id == 0 and iter_num % log_interval == 0:
                    ...
                    for idx, num_samples in num_samples_seen_per_pipeline.items():
                        writer.add_scalar(f'train/num_samples_seen_per_pipeline/{spam_subtype_mapping[idx]}',
                                          num_samples, iter_num)
                        print(
                            f"iter {iter_num}: num_samples_seen on 1 gpu for {spam_subtype_mapping[idx]}: {num_samples}")
            ...
            if gpu_id == 0 and is_bs_finetune:
...
if __name__ == "__main__":
    ...
    inference = "--infer" in sys.argv
    print("batch_size: ", batch_size)
    if inference:
        # infer_file = "/blob/rmayuranath/set_y_data_.tsv"
        # infer_file = "/blob/rmayuranath/fy23_cbspamv2_tokenized.tsv"
        infer_file = "/blob/rmayuranath/analysis/KRset_CleanBody.tsv"
        print("inference on file: ", infer_file)
        infer(model, batch_size, infer_file)
        exit(0)
