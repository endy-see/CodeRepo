...
class SpamMonoDataset(Dataset):
    def __init__(self, positive_file, positive_count=0, shuffle=True):
        # Store the paths to the positive and negative files and compute offsets
        self.positive_file = positive_file
        self.tokenizer = tokenizer
...
class SpamModelV2(nn.Module):
    ...
    def forward(self, url_x, x):
        ...
        # if finetune_dropout_replace_experiment:
        #     # strong dropout on url embedding
        #     url_x = F.dropout(url_x, p=0.5, training=self.training)
        ...
        if average_embeddings:
            ...
            # take first token output as embedding instead of mean
            x = x[:, 0, :]
...
eval_interval = 4000  # how often to evaluate the model on the validation set
...
def eval_binary_classifier(model, dataloader, iter_num, is_mikhail=False) -> None:
    ...
    if not is_mikhail:
        output_eval_file = os.path.join(out_dir, "eval_results_{}.txt".format(iter_num))
    else:
        output_eval_file = os.path.join(out_dir, "eval_results_mikhail_{}.txt".format(iter_num))
    ...
    for idx, (url_tokens, inputs, labels, pipelines) in enumerate(dataloader):
        ...
        pipelines_list.append(pipelines.view(-1).tolist())
        url_list.append(url_tokens.view(-1).tolist())
        outputs_exact.extend(outputs.tolist())
        labels_exact.extend(labels.tolist())
        ...
        if eval_mikhail_set and num_positives == num_positives_reqd and num_negatives >= num_positives * 500:
            break
    ...
    for idx in range(5):
        ...
        if neg_indices:
            if eval_mikhail_set:
                ...
                neg_indices_sub = random.sample(neg_indices, 500 * num_positives)
                ...
                neg_indices_sub = random.sample(neg_indices, min(len(output_list_sub) * 4, len(neg_indices))) if not (
                            eval_oct_leakage or eval_mikhail_set) else neg_indices
...
def train(model, batch_size, data_dir):
    # dummy operation to keep GPU busy so that it is not killed by Azure.
    # this is a hacky solution to the problem of Azure killing the GPU process after 2 hours of inactivity.

    # create a dummy tensor and keep it in GPU memory.
    # dummy_tensor = torch.zeros((45000, 45000)).to(device)
    ...
    if is_finetune:
        ...
        validation_file = "/scratch/singularity_webdata_ws01_eastus2_nfs/rmayuranath/data/v2/finetune/mikhail_bs/mikhail_set_validation.tsv"
        validation_set = SpamMonoDataset(positive_file=validation_file, shuffle=False)
        validation_loader = DataLoader(validation_set, batch_size=batch_size, shuffle=True, pin_memory=True,
                                       num_workers=4)
    ...
    while True:
        ...
        for batch in train_loader:
            ...
            if gpu_id == 0 and iter_num % eval_interval == 0:
                ...
                # eval on mikhail validation set also every 4th eval_interval
                if iter_num % (eval_interval * 2) == 0 and is_finetune:
