...
# build the command
command=("torchrun" "--standalone" "--nproc_per_node=4" "train_combined.py" "--tensorboard_dir" "$tensorboard_dir" "--freeze_encoder" "$freeze_encoder" "--enc_num_layers" "$enc_num_layers" "--average_embeddings" "$average_embeddings")
